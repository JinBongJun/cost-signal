...
### 1. 코드 개선 (보안/안정성) ✅ **부분 완료**
- [x] **타입 에러 수정** ✅ 완료
  - Webhook route 타입 에러 수정
  - Session 업데이트 타입 수정
- [x] **기능 개선** ✅ 완료
  - 이름 변경 기능 수정
  - 이메일 변경 기능 추가
  - 프로필 아바타 수정
- [ ] **환경 변수 검증 추가** (1-2시간) - 선택사항
  - `lib/env.ts` 생성
  - 앱 시작 시 필수 환경 변수 검증
  - 누락 시 명확한 에러 메시지
- [ ] **입력 검증 강화** (1-2시간) - 선택사항
...
- [ ] **타입 안정성 개선** (2-3시간) - 선택사항
  - 남은 `any` 타입을 구체적인 타입으로 변경
  - SessionUser, PaddleWebhookData 타입 정의

### 2. 인프라 설정 ✅ **완료**
- [x] **도메인 설정** ✅ 완료
  - 도메인 구매/연결 (`cost-signal.com`)
  - Vercel에 도메인 연결
  - SSL 인증서 확인
  - Cloudflare DNS 설정
  - Resend 도메인 인증
